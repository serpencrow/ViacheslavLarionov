...
public class DifferentElementsPage extends AbstractBasePage {
    ...
    public boolean isAllButtonTypeListDisplayed(final ButtonType buttonType) {
        List<WebElement> buttons = (ButtonType.CHECKBOX.equals(buttonType))
                ? checkboxes
                : radioButtons;

        return buttons.stream()
                .map(button -> isButtonTypeDisplayed(buttonType, button.getText()))
                .reduce(true, (a, b) -> a && b);
    }
    ...
    public boolean isFirstLogRowDisplayed() {
        return logSection.findElements(By.tagName("li")).get(0).isDisplayed();
    }
